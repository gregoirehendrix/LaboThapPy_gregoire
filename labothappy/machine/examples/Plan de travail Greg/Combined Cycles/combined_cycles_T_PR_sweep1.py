...
def plot_TQ_steam_generator(T_hot_C=800, PR=3.7567, save_dir=SAVE_DIR):
    """
    Diagrammes T–Q du générateur de vapeur et du réchauffeur
    (cycle combiné Air simple + Steam Siemens).

    Les figures sont prévues pour être intégrées côte à côte
    en subfigures LaTeX (≈ 0.48\\textwidth).
    """

    # ── Compensation police pour subfigures LaTeX ─────────────────────
    SCALE = 1.31  # ≈ 1.77
    ...
    with plt.rc_context({
        ...
        'xtick.labelsize' : 24 * SCALE,
        'ytick.labelsize' : 24 * SCALE,
        ...
        cycle, comp, turb, heater = brayton_simple(
            0.75, 0.85, 0.90, HSource, AirInlet, P_low_b, P_high_b
        )
        ...
        pump, eco, eva, sh, rh, turb_hp, turb_ic, cond = _solve_siemens(
            eta_pump=0.80, eta_turb=0.93, eta_hx=0.95,
            pinch_hx=5.0, pinch_cond=5.0,
            T_hot_K=T_exhaust, T_amb_K=T_amb,
            m_dot_air=20.0, P_hot=1.01325e5,
            P_low=0.095e5, P_high=160e5, P_rh=38.41e5, m_dot_st=1.0
        )

        # ── 3. Chaleurs [kW] ──────────────────────────────────────────
        m_st   = pump.su.m_dot
        q_eco  = m_st * (eco.ex_C.h - eco.su_C.h) / 1000
        q_eva  = m_st * (eva.ex_C.h - eva.su_C.h) / 1000
        q_sh   = m_st * (sh.ex_C.h  - sh.su_C.h)  / 1000
        q_rh   = m_st * (rh.ex_C.h  - rh.su_C.h)  / 1000
        Q_main = q_eco + q_eva + q_sh

        # ── 4. Points T–Q générateur ──────────────────────────────────
        Q_axis = [0, q_eco, q_eco, q_eco + q_eva, q_eco + q_eva, Q_main]
        ...
        # ── FIGURE 1 : Générateur de vapeur ───────────────────────────
        fig1, ax1 = plt.subplots(figsize=(14, 10))

        ax1.plot(Q_axis, T_cold,      color='steelblue', lw=2.5,
                 marker='o', ms=6, label='Steam (cold side)')
        ax1.plot(Q_axis, T_hot_curve, color='firebrick',  lw=2.5,
                 marker='o', ms=6, label='Exhaust air (hot side)')

        for Q_lim in [q_eco, q_eco + q_eva]:
            ax1.axvline(Q_lim, color='gray', ls='--', lw=1.0, alpha=0.5)

        y_zone = min(min(T_cold), min(T_hot_curve)) + 25
        ax1.text(q_eco / 2, y_zone, 'ECO', ha='center', fontsize=22 * SCALE)
        ax1.text(q_eco + q_eva + q_sh / 2, y_zone, 'SH', ha='center', fontsize=22 * SCALE)

        dT    = [h - c for h, c in zip(T_hot_curve, T_cold)]
        idx_p = int(np.argmin(dT))

        """ax1.annotate(
            rf'$\Delta T_{{\mathrm{{pinch}}}}$ = {dT[idx_p]:.1f} °C',
            xy=(Q_axis[idx_p], T_cold[idx_p]),
            xytext=(Q_axis[idx_p] + Q_main * 0.08, T_cold[idx_p] - 40),
            arrowprops=dict(arrowstyle='->', lw=1.5),
            fontsize=20 * SCALE,
            bbox=dict(boxstyle='round,pad=0.3', fc='white', alpha=0.8),
        )"""

        ax1.set_xlabel(r'Cumulative heat transfer $\dot{Q}$ [kW / kg$_{\mathrm{steam}}$ s$^{-1}$]')
        ax1.set_ylabel(r'Temperature $T$ [°C]')
        ax1.legend(loc='best')
        fig1.tight_layout()

        path1 = os.path.join(save_dir, 'fig_TQ_steam_generator.pdf')
        fig1.savefig(path1, bbox_inches='tight')
        plt.close(fig1)

        # ── FIGURE 2 : Réchauffeur ────────────────────────────────────
        fig2, ax2 = plt.subplots(figsize=(14, 10))

        ax2.plot([0, q_rh], [rh.su_C.T - 273.15, rh.ex_C.T - 273.15],
                 color='steelblue', lw=2.5, marker='o', ms=8,
                 label='Steam (cold side)')
        ax2.plot([0, q_rh], [rh.ex_H.T - 273.15, rh.su_H.T - 273.15],
                 color='firebrick', lw=2.5, marker='o', ms=8,
                 label='Exhaust air (hot side)')

        ax2.set_xlabel(r'Cumulative heat transfer $\dot{Q}$ [kW / kg$_{\mathrm{steam}}$ s$^{-1}$]')
        ax2.set_ylabel(r'Temperature $T$ [°C]')
        ax2.legend(loc='best')
        fig2.tight_layout()

        path2 = os.path.join(save_dir, 'fig_TQ_reheater.pdf')
...
if __name__ == "__main__":
    ...
    # ── Température de retour Steam (condenseur ~45°C + pinch 10K) ───
    T_ret_steam = PropsSI('T', 'P', 0.095e5, 'Q', 0, 'Water') + 10.0
    ...
    for PR in PR_values:
        ...
        for T_hot_C in T_hot_values_C:
            ...
            try:
                r  = run_combined("Simple", params,
                                  N_towers=N_towers, verbose=False)
                br = r['br']

                Q_avail_real = available_heat(br['h_exhaust'], T_ret_steam)

                bp = bottom_steam_siemens(r['T_bottom_in'], Q_avail_real, T_amb)
                if bp is None:
                    print("    → Fallback bottom_steam simplifié")
                    bp = bottom_steam(r['T_bottom_in'], Q_avail_real)

                if bp is not None:
                    W_combined = br['W_net'] + bp['W_net']
                    eta_comb   = W_combined / br['Q_total']
                    gain_pp    = (eta_comb - br['eta']) * 100
                    eta_steam  = bp['eta'] * 100
                    W_steam    = bp['W_net']
                else:
                    W_combined = br['W_net']
                    eta_comb   = br['eta']
                ...
                print(f"PR={PR:.4f} | T={T_hot_C}°C | {hot_fluid:9s} | "
                      ...
                      f"T_exh={br['T_exhaust']-273.15:.1f}°C | "
                      f"η_st={eta_steam:5.2f}% | "
    ...
    df.to_csv('sweep_simple_steam.csv', index=False)
    print("\nSauvegardé dans sweep_simple_steam.csv")
