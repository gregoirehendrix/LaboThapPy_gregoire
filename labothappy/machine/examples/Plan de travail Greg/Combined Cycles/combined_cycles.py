...
if __name__ == "__main__":
    ...
    print(f"Source chaude : {hot_fluid} à {T_hot_su-273.15:.0f} °C\n")
...
if __name__ == "__main__":
    ...
    # ── Configurations optimales issues du sweep ──────────────────────────
    #   Récupéré     : PR_opt = 5   → η_comb = 51.72 %  (ORC Toluène)
    #   RH+IC        : PR_opt = 7   → η_comb = 50.81 %  (ORC Cyclopentane)
    optimal_cases = [
        ("Recuperated",      5.0),
        ("Recuperated_RH_IC", 7.0),
    ]

    results = []
    for case, PR in optimal_cases:
        P_high = P_low * PR
