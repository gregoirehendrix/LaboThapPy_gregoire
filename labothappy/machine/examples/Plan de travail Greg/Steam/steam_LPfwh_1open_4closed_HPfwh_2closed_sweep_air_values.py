...
if __name__ == "__main__":
    ...
    # --- Tableau T_pb_in [°C] issu du sweep Brayton simple + piping ---
    PR_values      = [3.0000, 3.7567, 4.4373, 4.8853, 5.0000]
    T_hot_values_C = [600, 650, 700, 750, 800, 850, 900, 950, 1000]

    T_pb_in_C = {
        600:  [371.11, 343.00, 323.22, 312.18, 309.56],
        650:  [407.42, 377.80, 356.93, 345.28, 342.51],
        700:  [443.85, 412.72, 390.79, 378.54, 375.62],
        750:  [480.36, 447.76, 424.78, 411.92, 408.86],
        800:  [516.95, 482.89, 458.85, 445.41, 442.21],
        850:  [553.59, 518.09, 493.02, 478.98, 475.65],
        900:  [590.28, 553.34, 527.24, 512.63, 509.15],
        950:  [627.01, 588.64, 561.51, 546.33, 542.72],
        1000: [663.77, 623.97, 595.83, 580.07, 576.33],
    }

    def _adjust_pressures(T_salt_K):
        T_su = T_salt_K - pinch_hx
        T_sat_max = min(T_su - 5.0, T_CRIT - 5.0)
        if T_sat_max <= 273.15 + 50:
            return None
        try:
        ...
        P_rh      = P_high * (P_RH / P_HIGH)
        P_rh      = max(P_rh, P_LOW * 10)
        P_hp_ext  = min(P_HP_EXT_NOM, P_high  * 0.28)
        P_fwh5    = min(P_FWH5_NOM,   P_rh    * 0.50)
        P_dea     = min(P_DEA_NOM,    P_fwh5  * 0.60)
        P_lp4     = min(P_LP4_NOM,    P_dea   * 0.52)
        P_lp3     = min(P_LP3_NOM,    P_lp4   * 0.52)
        P_lp2     = min(P_LP2_NOM,    P_lp3   * 0.43)
        P_lp1     = min(P_LP1_NOM,    P_lp2   * 0.32)
        P_lp1     = max(P_lp1, P_LOW * 3.0)
        return dict(P_high=P_high, P_rh=P_rh,
                    P_hp_ext=P_hp_ext, P_fwh5=P_fwh5, P_dea=P_dea,
                    P_lp4=P_lp4, P_lp3=P_lp3, P_lp2=P_lp2, P_lp1=P_lp1)

    # --- Cache sur températures uniques ---
    all_temps = sorted(set(t for row in T_pb_in_C.values() for t in row))
    cache = {}

    print(f"Sweep vapeur sur {len(all_temps)} températures uniques...\n")

    for T in all_temps:
        T_K = T + 273.15
        p   = _adjust_pressures(T_K)
        if p is None:
            print(f"  T={T:.2f}°C → SKIP (T trop basse)")
            cache[T] = (np.nan, np.nan)
            continue
        try:
            pump, eco, eva, sh, rh, turb_hp, turb_ic, cond = solve_cycle(
                eta_pump, eta_turb, eta_hx, pinch_hx, pinch_cond,
                T_K, P_SALT, M_DOT_SALT, F_RH,
                CSource, P_LOW, p['P_high'], p['P_rh'], M_DOT_ST,
            )
            perf = compute_fwh(
                pump, eco, eva, sh, rh, turb_hp, turb_ic, cond,
                eta_pump, eta_turb, p['P_high'], P_LOW,
                p['P_hp_ext'], p['P_fwh5'], p['P_dea'],
                p['P_lp4'], p['P_lp3'], p['P_lp2'], p['P_lp1'],
                eta_g, pinch_cfwh=pinch_cfwh,
            )
            eta = perf['eta_th'] * 100
            W   = perf['W_net']
            cache[T] = (eta, W)
            print(f"  T={T:.2f}°C  P_high={p['P_high']/1e5:.1f}bar  "
                  f"η={eta:.2f}%  W={W:.3f} kW/kg")
        except Exception as e:
            print(f"  T={T:.2f}°C → FAILED : {e}")
            cache[T] = (np.nan, np.nan)

    # --- Matrices résultats ---
    rows_eta, rows_wnet = [], []
    for T_hot_C in T_hot_values_C:
        row_eta  = {'T_hot_C': T_hot_C}
        row_wnet = {'T_hot_C': T_hot_C}
        for i, PR in enumerate(PR_values):
            T = T_pb_in_C[T_hot_C][i]
            eta, W = cache[T]
            row_eta[round(PR, 4)]  = round(eta, 2) if not np.isnan(eta) else np.nan
            row_wnet[round(PR, 4)] = round(W,   3) if not np.isnan(W)   else np.nan
        rows_eta.append(row_eta)
        rows_wnet.append(row_wnet)

    df_eta  = pd.DataFrame(rows_eta).set_index('T_hot_C')
    df_wnet = pd.DataFrame(rows_wnet).set_index('T_hot_C')
    df_eta.columns.name  = 'PR'
    df_wnet.columns.name = 'PR'

    print("\n=== η_th cycle vapeur [%] (rows=T_hot [°C], cols=PR) ===")
