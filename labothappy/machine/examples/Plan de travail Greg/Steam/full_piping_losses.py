...
# =============================================================================
# PARAMETERS — corrected block  (replace the existing one in full_piping_losses.py)
# =============================================================================
# Back-calculated from John Cockerill insulation data (100 W/m² target)
# Cylindrical model : q = ΔT / (R_ins + R_conv) / (π D_total)
# ---------------------------------------------------------------------------

T_SWITCH_INS = (565 + 300) / 2   # = 432.5°C  — switch threshold

# ── HOT piping — 565°C, Rockwool ProRox 960, target 100 W/m² ────────────────
# Source: JC table (DN32_100 → DN125_100 identifiers)
E_INSULATION_HOT = {
    '1 1/4"': 0.170,   # DN32  — 170 mm → 98.27 W/m²  (JC)
    '2"'    : 0.180,   # DN50  — 180 mm → 100   W/m²  (JC)
    '2 1/2"': 0.190,   # DN65  — 190 mm → 94.71 W/m²  (JC)
    '3"'    : 0.200,   # DN80  — 200 mm → 100   W/m²  (JC)
    '3 1/2"': 0.200,   # DN90  — 200 mm → 100   W/m²  (JC)  ← NEW
    '4"'    : 0.210,   # DN100 — 210 mm → 99.74 W/m²  (JC)  ← NEW
    '5"'    : 0.210,   # DN125 — 210 mm → 100   W/m²  (JC)  ← NEW
}
LAMBDA_HOT = 0.078    # W/m·K — back-calculated from JC data (unchanged)

# ── COLD piping — 300°C, Rockwool ProRox 960, target 100 W/m² ───────────────
# Source: JC table (DN32_100 → DN125_100 identifiers)
E_INSULATION_COLD = {
    '1 1/4"': 0.080,   # DN32  —  80 mm → 100   W/m²  (JC)
    '2"'    : 0.080,   # DN50  —  80 mm  (JC, calculated heat loss not shown)
    '2 1/2"': 0.090,   # DN65  —  90 mm → 99.99 W/m²  (JC)
    '3"'    : 0.090,   # DN80  —  90 mm → 100   W/m²  (JC)
    '3 1/2"': 0.090,   # DN90  —  90 mm → 100   W/m²  (JC)  ← NEW
    '4"'    : 0.100,   # DN100 — 100 mm → 100   W/m²  (JC)  ← NEW
    '5"'    : 0.100,   # DN125 — 100 mm → 100   W/m²  (JC)  ← NEW
}
LAMBDA_COLD = 0.060   # W/m·K — CORRECTED (was 0.055; back-calc from JC: ~0.059–0.060)
...
# ── Pipe diameter tables ─────────────────────────────────────────────────────
# DIAM_TABLE : number of upstream units → NPS label
# Extended to cover charges 1–14 (CAP up to 10 in 2-D sweep)
DIAM_TABLE = {
    1 : '1 1/4"',   # DN32   — 1 unit  →  5.99 kg/s
    2 : '2"',        # DN50   — 2 units → 11.99 kg/s
    3 : '2 1/2"',   # DN65   — 3 units → 17.98 kg/s
    4 : '3"',        # DN80   — 4 units → 23.98 kg/s
    5 : '3"',        # DN80   — 5 units → 29.97 kg/s
    6 : '3 1/2"',   # DN90   — 6 units → 35.97 kg/s  ← NEW
    7 : '3 1/2"',   # DN90   — 7 units → 41.96 kg/s  ← NEW
    8 : '4"',        # DN100  — 8 units → 47.96 kg/s  ← NEW
    9 : '4"',        # DN100  — 9 units → 53.95 kg/s  ← NEW
    10: '5"',        # DN125 — 10 units → 59.94 kg/s  ← NEW
    11: '5"',        # DN125 — 11 units → 65.94 kg/s  ← NEW
    12: '5"',        # DN125 — 12 units → 71.93 kg/s  ← NEW
    13: '5"',        # DN125 — 13 units → 77.93 kg/s  ← NEW
    14: '5"',        # DN125 — 14 units → 83.92 kg/s  ← NEW
}

# DIAMETER_M : NPS label → outer pipe diameter [m]  (ASME B36.10 / EN 10220)
DIAMETER_M = {
    '1 1/4"': 0.0422,   # DN32
    '2"'    : 0.0603,   # DN50
    '2 1/2"': 0.0730,   # DN65
    '3"'    : 0.0889,   # DN80
    '3 1/2"': 0.1016,   # DN90   ← NEW
    '4"'    : 0.1143,   # DN100  ← NEW
    '5"'    : 0.1413,   # DN125  ← NEW
}
...
def _resolve_insulation(diam_str, T_fluid_C):
    """
    Select insulation thickness and conductivity based on fluid temperature.
    Switches at T_SWITCH_INS = 432.5°C (midpoint between 565°C and 300°C).
    """
    if T_fluid_C > T_SWITCH_INS:
        e = E_INSULATION_HOT.get(diam_str, 0.200)
        lam = LAMBDA_HOT
    else:
        e = E_INSULATION_COLD.get(diam_str, 0.090)
        lam = LAMBDA_COLD
    return 0.0, e, lam, lam
...
def compute_cascaded_temperatures(parent, coords, pipe_info, children,
    ...
    def _segment(child, par, T_ch):
        ...
        e_s, e_r, ls, lr = _resolve_insulation(diam, T_ch - 273.15)   # <-- diam string, plus D_k
...
def print_insulation_summary():
    print(f"\n  Insulation mode : per-diameter (rockwool only)")
    print(f"  h_ext = {H_EXT:.1f} W/m²·K")
    print(f"  {'Diam':10s}  {'D_ext[mm]':10s}  {'e_ins[mm]':10s}  "
          f"{'D_total[mm]':12s}  {'U_eq[W/m²K]':12s}")
    for diam, D_ext in DIAMETER_M.items():
        e_s, e_r, ls, lr = _resolve_insulation(diam, 565)
        U_eq, D_total = compute_U_and_Dtotal(D_ext, e_s, e_r, ls, lr, H_EXT)
        e_total = (e_s + e_r) * 1e3
        print(f"  {diam:10s}  {D_ext*1e3:10.1f}  {e_total:10.1f}  "
              f"{D_total*1e3:12.1f}  {U_eq:12.4f}")
