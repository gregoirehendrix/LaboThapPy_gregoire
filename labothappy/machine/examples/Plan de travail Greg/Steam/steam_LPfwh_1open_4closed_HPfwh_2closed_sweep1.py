...
if __name__ == "__main__":
    ...
    # --- Sweep 545–565°C, pas 1°C ---
    all_temps = list(range(545, 566))
    cache = {}

    print(f"Sweep vapeur de 545°C à 565°C ({len(all_temps)} points)...\n")

    for T in all_temps:
        T_K = T + 273.15
        p   = _adjust_pressures(T_K)
        if p is None:
            print(f"  T={T}°C → SKIP (T trop basse)")
        ...
        try:
            ...
            W   = perf['W_net']
            cache[T] = (eta, W)
            print(f"  T={T}°C  P_high={p['P_high']/1e5:.1f}bar  "
                  f"η={eta:.2f}%  W={W:.3f} kW/kg")
        except Exception as e:
            print(f"  T={T}°C → FAILED : {e}")
            cache[T] = (np.nan, np.nan)

    # --- Tableau de résultats ---
    print("\n=== Résultats sweep 545–565°C ===")
    print(f"{'T (°C)':<10} {'η_th (%)':<12} {'W_net (kW/kg)':<15}")
    print("-" * 37)
    for T in all_temps:
        eta, W = cache[T]
        if np.isnan(eta):
            print(f"  {T:<8} {'FAILED':<12} {'FAILED'}")
        else:
            print(f"  {T:<8} {eta:<12.2f} {W:<15.3f}")

    # --- Export CSV ---
    df = pd.DataFrame([
        {'T_C': T, 'eta_th_%': cache[T][0], 'W_net_kW_per_kg': cache[T][1]}
        for T in all_temps
    ])
    df.to_csv('steam_sweep_545_565.csv', index=False)
    print("\nSauvegardé dans steam_sweep_545_565.csv")