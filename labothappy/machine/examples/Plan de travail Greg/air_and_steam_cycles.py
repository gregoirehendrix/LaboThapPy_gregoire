# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 10:26:48 2026
...
Topology:
  ...
  Oil -> SH (Oil/Steam) [parallel]
      ...
      -> Reheater (Oil/Steam) [parallel, optional]
  Steam -> Expander HP -> [Reheater] -> Expander LP -> Condenser -> Pump -> Evap
...
W_net_target   = 500e6  # W — target net power output
# ===========================================================================
...
def run_rankine_rh(T_oil_su_K, P_high, T_sh_ex_K, P_rh, T_rh_ex_K):
    ...
    HP turbine: P_high -> P_rh
    Reheater  : steam reheated to T_rh_ex at P_rh (oil at T_oil_su)
    LP turbine: P_rh -> P_cond
    """
    try:
    ...
    try:
        ...
        # reheater: steam reheated to T_rh_ex at P_rh
...
def make_bounds(T_ex_turb_K):
    ...
    if USE_REHEAT:
        base += [
            (1.0,  100.0),      # P_rh_bar  (must be < P_high, enforced in evaluate)
            (0.0,  100.0),      # dT_rh [K]
        ]
    return base
...
def evaluate(x, brayton, bounds):
    ...
    if USE_REHEAT:
        ...
        if T_rh_ex_K >= T_oil_su_K - PINCH_MIN: return 0.0, None

        r_st = run_rankine_rh(T_oil_su_K, P_high, T_sh_ex_K, P_rh, T_rh_ex_K)
        if r_st is None: return 0.0, None
        x_lp_ex = steam_quality(r_st['h_lp_ex'])
        if x_lp_ex < X_MIN: return 0.0, None
        h_exp_ex_key = 'h_lp_ex'
    else:
                ...
                for dT in [10.0, 30.0]:
                    for P_rh in P_rh_list:
                        for dT_rh in dT_rh_list:
                            ...
                            label = (f"T_oil={T_oil:.0f} T_ret={T_ret:.0f} "
                                     f"P={P:.0f} dT={dT:.0f}")
                            if USE_REHEAT:
...
def find_x0(brayton, bounds):
    ...
    if USE_REHEAT:
        P_rhs   = [2.0, 5.0, 10.0, 20.0]
        dT_rhs  = [10.0, 20.0, 40.0]
        for T_oil in T_oils:
            for T_ret in T_rets:
                for P in P_highs:
                    for dT in dT_shs:
...
def print_results(x_opt, eta_opt, det, brayton, m_dot_air):
    ...
    if USE_REHEAT:
        print(f"  Pinch RH          = {x_opt[0] - (det['T_rh_ex'] - 273.15):.1f}  K")
    print(f"{'─'*54}")
    print(f"  W_net_Brayton  = {brayton['W_net']/1e3:7.2f} kW/kg/s  →  {brayton['W_net']/1e6*m_dot_air:6.2f} MW")
    if USE_REHEAT:
        r_st = det['r_st']
        print(f"  W_net_ST_HP    = {r_st['W_hp']/1e3:7.2f} kW/kg/s_wf")
        print(f"  W_net_ST_LP    = {r_st['W_lp']/1e3:7.2f} kW/kg/s_wf")
